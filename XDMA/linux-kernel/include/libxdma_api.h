 ...
 * @dev_hndl: corresponding handler
 * @channel: channel number (< channel_max)
 *	== channel_max means libxdma can pick any channel available:q

 * @write : boolean, true for WRITING to device, else READING from device
 * @ep_addr: offset into the DDR/BRAM memory to read from or write to
 * @sgt: the scatter-gather list of data buffers
 * @dma_mapped: ???
 * @timeout_ms: timeout in milli-seconds, *currently ignored
